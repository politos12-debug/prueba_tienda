...
async function initAdmin() {
    ...
    waitForSupabaseAdmin(async function() {
        ...
        if (typeof NewsletterEditor !== 'undefined') NewsletterEditor.init();
...
function cambiarSeccion(section) {
    ...
    if (section === 'newsletter') {
        loadNewsletterData();
        if (typeof NewsletterEditor !== 'undefined') {
            NewsletterEditor.loadSubscriberCount();
            NewsletterEditor.loadProducts();
        }
    }
    ...
    // Si cambió a marcas, cargar datos
    if (section === 'marcas') {
        if (typeof cargarMarcasAdmin === 'function') {
            cargarMarcasAdmin();
        }
    }
}
...
function renderizarTablaProductos(productos) {
    ...
    productosPagina.forEach(producto => {
        ...
        const sinStock = producto.stock === 0 || producto.stock === '0';
        
        // Rojo solo si está activo y sin stock; gris si está desactivado
        let estilo = '';
        let stockTexto = producto.stock;
        if (!esActivo) {
            estilo = 'style="background-color: #f0f0f0; color: #999;"';
            stockTexto = producto.stock + (sinStock ? ' AGOTADO' : '');
        } else if (sinStock) {
            estilo = 'style="background-color: #ffe6e6; color: #c00; font-weight: 600;"';
            stockTexto = '0 AGOTADO';
        }
        ...
        if (producto.imagen_url) {
            try {
                const imgs = typeof producto.imagen_url === 'string' ? JSON.parse(producto.imagen_url) : producto.imagen_url;
                thumbUrl = Array.isArray(imgs) ? (imgs[0] || '') : producto.imagen_url;
            } catch (e) {
                thumbUrl = producto.imagen_url;
            }
        }
        
        const opacidad = esActivo ? '' : 'opacity:0.5;';

        row.innerHTML = `
            <td style="text-align:center;padding:4px;${opacidad}">
                ${thumbUrl ? `<img src="${thumbUrl}" alt="${producto.nombre}" style="width:40px;height:40px;object-fit:cover;border-radius:4px;" onerror="this.style.display='none'">` : '<span style="color:#ccc;font-size:0.8em;">—</span>'}
            </td>
            <td ${estilo}>${producto.nombre}</td>
            <td ${estilo}>€${parseFloat(producto.precio).toFixed(2)}</td>
            <td ${estilo}>${producto.categoria}</td>
            <td ${estilo}>${stockTexto}</td>
            <td style="text-align:center;">
                <label class="toggle-activo" style="position:relative;display:inline-block;width:44px;height:24px;cursor:pointer;">
                    <input type="checkbox" ${esActivo ? 'checked' : ''} onchange="toggleActivoProducto(${producto.id}, this.checked)" style="opacity:0;width:0;height:0;">
                    <span style="position:absolute;top:0;left:0;right:0;bottom:0;background:${esActivo ? '#4caf50' : '#ccc'};border-radius:24px;transition:.3s;"></span>
                    <span style="position:absolute;top:2px;left:${esActivo ? '22px' : '2px'};width:20px;height:20px;background:#fff;border-radius:50%;transition:.3s;box-shadow:0 1px 3px rgba(0,0,0,.3);"></span>
                </label>
            </td>
            <td>
...
async function toggleActivoProducto(id, activo) {
    ...
    if (idx !== -1) {
        productosGlobal[idx].activo = activo;
    }

    // Re-renderizar para reflejar cambio de opacidad
    const terminoBusqueda = document.getElementById('buscar-productos')?.value?.toLowerCase();
    if (terminoBusqueda) {
        filtrarProductos();
    } else {
...
async function abrirFormularioProducto() {
    ...
    // Cargar categorías y marcas dinámicamente desde la BD
    await cargarCategoriasProductoForm();
    await cargarMarcasProductoForm();
...
// ========== CATEGORÍAS DINÁMICAS DEL FORMULARIO ==========

async function cargarCategoriasProductoForm(categoriaSeleccionada) {
    const select = document.getElementById('prod-categoria');
    if (!select || !supabaseAdmin) return;
    ...
    try {
        ...
        // Disparar carga de subcategorías para el valor actual
        cargarSubcategorias(select.value);
        mostrarSeccionTallas(select.value);
    } catch (err) {
        console.error('Error cargando categorías del formulario:', err.message);
    }
}

// ========== MARCAS EN FORMULARIO PRODUCTO ==========

async function cargarMarcasProductoForm(marcaSeleccionada) {
    ...
    try {
        ...
        console.error('Error cargando marcas del formulario:', err.message);
    }
}
...
async function editarProducto(id) {
    try {
        ...
        // Cargar categorías y marcas dinámicamente y seleccionar las del producto
        await cargarCategoriasProductoForm(data.categoria || '');
        await cargarMarcasProductoForm(data.marca || '');
        ...
        // Agregar listener a cambio de categoría
        if (categoria) {
...
async function guardarProducto(e) {
    try {
        ...
        const datos = {
            ...
            marca: (marcaSelect && marcaSelect.value) ? marcaSelect.value : null
        };
        ...
        // Guardar stock anterior para detectar cambios (solo al editar)
        ...
        if (productoEditar) {
            const { data: prodActual } = await supabaseAdmin
                ...
                .single();
            stockAnterior = prodActual ? prodActual.stock : null;
        }
        ...
        // Notificar cambio de stock a usuarios con producto en favoritos
        if (productId && stockAnterior !== null && stockAnterior !== datos.stock) {
            fetch('/api/check-stock-favorites', {
                method: 'POST',
            ...
            }).catch(err => console.warn('[admin] Error notificando cambio de stock:', err));
        }
...
// sendNewsletterEmail ahora gestionado por NewsletterEditor en newsletter-editor.js
