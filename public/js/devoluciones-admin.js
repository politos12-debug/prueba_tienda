...
function abrirDevolucion(devolucion) {
    ...
    const descargarDiv = document.getElementById('dev-descargar');
    
    if (devolucion.estado === 'procesado') {
        acciones.style.display = 'flex';
        descargarDiv.style.display = 'none';
    } else if (devolucion.estado === 'confirmada') {
        acciones.style.display = 'none';
        descargarDiv.style.display = 'block';
    } else {
        acciones.style.display = 'none';
        descargarDiv.style.display = 'none';
    }
...
async function descargarFacturaDevolucioAdmin() {
    if (!devolucionActual) return;
    
    try {
        const pedidoId = devolucionActual.pedido_id;
        const url = `/api/admin/download-invoice?id=${pedidoId}&type=devolucio`;
        ...
        link.href = linkUrl;
        link.download = `factura-devolucion-${pedidoId}.pdf`;
        ...
        mostrarModalNotificacion(
            'Error',
            'No se pudo descargar la factura de devolución',
            'error'
        );
    }
}
