...
async function handleLogin(e) {
    ...
    const btnTexto = btn ? btn.querySelector('.btn-texto') : null;
    ...
    try {
        ...
        console.log('Login exitoso, actualizando UI');
        updateAuthUI();

        // Sincronizar carrito con BD después de login
        console.log('Sincronizando carrito después de login...');
        await sincronizarCarritoDesdeAuth();

        // Si estamos en /cuenta, redirigir a inicio
...
async function handleRegister(e) {
    ...
    function showRegError(msg) {
        const el = document.getElementById('register-error') || createErrorContainer();
        el.textContent = msg;
        el.style.display = 'block';
    }

    if (!nombre || !email || !password || !confirmPassword) {
        showRegError('Por favor rellena todos los campos requeridos');
        return;
    }
    ...
    if (password.length < 6) {
        showRegError('La contraseña debe tener al menos 6 caracteres');
        return;
    }
    ...
    try {
        ...
        const errorContainer = document.getElementById('register-error') || createErrorContainer();
        errorContainer.textContent = 'Error al registrarse: ' + error.message;
        errorContainer.style.display = 'block';
...
function updateAuthUI() {
    ...
    if (currentUserData) {
        ...
        if (isMobile) {
            // Móvil: auth-button está oculto por CSS, opciones van en nav
            authBtn.innerHTML = '';
            
            // Ocultar "Iniciar Sesión" del menú (usuario ya está logueado)
            const mobileIniciar = document.getElementById('mobile-iniciar-sesion');
            if (mobileIniciar) {
                mobileIniciar.style.setProperty('display', 'none', 'important');
            }
            ...
            // Agregar items al menú móvil (nav ul)
            ...
            navUl.appendChild(misComprasLi);
            ...
            authBtn.classList.remove('logged-in-mobile');
        ...
        // Limpiar items móviles del estado logueado
        const oldMiPerfil = document.getElementById('mobile-mi-perfil');
        const oldMisCompras = document.getElementById('mobile-mis-compras');
        const oldAdmin = document.getElementById('mobile-admin-link');
        const oldLogout = document.getElementById('mobile-logout-link');
        ...
        // Restaurar "Iniciar Sesión" en menú móvil
        const mobileIniciar = document.getElementById('mobile-iniciar-sesion');
        if (mobileIniciar) mobileIniciar.style.removeProperty('display');
        
        // Desktop: mostrar icono de login
        authBtn.classList.remove('logged-in-mobile');
        authBtn.style.display = '';
        authBtn.innerHTML = `<a href="/cuenta" class="auth-icon" title="Iniciar Sesión"><svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg></a>`;
    }
}
