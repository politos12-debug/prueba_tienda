// ========== RELOJERÍA PAGE SCRIPT ==========
// Muestra productos de la categoría Relojes con filtros modales
...
function limpiarTodosFiltros() {
    ...
    document.querySelectorAll('#filtro-marca .subcategoria-btn').forEach(function(b) { b.classList.remove('active'); });
    var btnTodos = document.querySelector('#filtro-marca .subcategoria-btn:first-child');
...
function aplicarFiltrosYMostrar() {
    ...
    if (marcaSeleccionada) {
        productos = productos.filter(function(p) {
            return p.marca && p.marca.toLowerCase() === marcaSeleccionada.toLowerCase();
        });
    }
...
// ========== CARGAR MARCAS COMO FILTROS ==========

async function cargarMarcasFiltro() {
    var contenedor = document.getElementById('filtro-marca');
    ...
    try {
        ...
        if (result.error || !result.data || result.data.length === 0) {
            var seccion = document.getElementById('filtro-marca-seccion');
        ...
        btnTodas.className = 'subcategoria-btn active';
        ...
        result.data.forEach(function(marca) {
...
function renderizarProductos(productos) {
    ...
    productos.forEach(function(producto) {
        ...
        card.innerHTML =
            badgeOferta +
            '<img src="' + (imagenURL || 'https://via.placeholder.com/250x200?text=Reloj') + '" alt="' + producto.nombre + '" style="width:100%;height:200px;object-fit:cover;' + (!hayStock ? 'opacity:0.5;' : '') + '">' +
        ...
        if (btnAgregar && hayStock) {
            btnAgregar.addEventListener('click', function(e) {
                ...
                window.agregarAlCarritoRelojeria(item);
            });
        }
...
window.agregarAlCarritoRelojeria = function(producto) {
...
function mostrarMensajeErrorStock(mensaje) {
    var errorDiv = document.getElementById('error-stock-relojeria');
    if (!errorDiv) {
        errorDiv = document.createElement('div');
        errorDiv.id = 'error-stock-relojeria';
...
async function cargarProductosRelojeria() {
    var intentos = 0;
    ...
    if (result.error) {
        console.error('Error al cargar relojes:', result.error);
        return;
    }

    productosActuales = result.data || [];

    await cargarMarcasFiltro();
