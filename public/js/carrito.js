...
let _updatingCartQuantity = false;

// Actualizar cantidad - recibe cantidad absoluta (desde input o desde incrementar/decrementar)
async function actualizarCantidad(index, nuevaCantidad) {
    // Prevenir llamadas concurrentes (spam de botones)
    if (_updatingCartQuantity) return;
    _updatingCartQuantity = true;

    // Deshabilitar TODOS los botones de cantidad inmediatamente
    document.querySelectorAll('.qty-btn, .qty-btn-small, .btn-cantidad').forEach(btn => btn.disabled = true);
    ...
    try {
        ...
        // Si no hay cambio, nada que hacer
        ...
        if (diferencia > 0) {
            ...
            const res = await fetch('/api/add-to-cart-validated', {
                ...
                body: JSON.stringify({ productId: item.id, cantidad: diferencia })
            });

            if (!res.ok) {
                const data = await res.json().catch(() => ({}));
                mostrarErrorStock(data.error || `No hay suficiente stock. Disponible: ${data.stockDisponible || 0}`);
                return;
            }

            console.log('[actualizarCantidad] +' + diferencia + ' aprobado por servidor para', item.nombre);

        } else {
            // DECREMENTAR: Restaurar stock en servidor
            await fetch('/api/update-cart-stock', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ productId: item.id, cantidad: Math.abs(diferencia), accion: 'sumar' })
            });

            console.log('[actualizarCantidad] ' + diferencia + ' para', item.nombre);
        }

        // Servidor aprobó → ahora sí actualizar localStorage
        const freshCart = JSON.parse(localStorage.getItem('carrito') || '[]');
        if (freshCart[index]) {
            freshCart[index].cantidad = cantidad;
        ...
        window.dispatchEvent(new CustomEvent('carritoActualizado', {
            detail: { carrito: freshCart, productoId: item.id }
        }));
...
async function incrementarCantidad(index) {
    if (_updatingCartQuantity) return;
    const carritoFresco = JSON.parse(localStorage.getItem('carrito') || '[]');
    const item = carritoFresco[index];
    if (!item) return;
    await actualizarCantidad(index, item.cantidad + 1);
}
