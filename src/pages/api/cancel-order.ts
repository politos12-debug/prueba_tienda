            ...
            for (const item of items) {
                if (item.product_id) {
                    ...
                    if (!getError && productData) {
                        const stockAnterior = productData.stock || 0;
                        const nuevoStock = stockAnterior + (item.cantidad || 1);
                        ...
                        if (!updateStockError) {
                            ...
                            // Notificar cambio de stock (fire-and-forget)
                            notifyStockChange(item.product_id, stockAnterior, nuevoStock).catch(() => {});
                        } else {
