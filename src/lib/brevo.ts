            ...
            <body>
                <div class="container">
                    ...
                    <div class="content">
                        ...
                        <center>
                            <a href="https://galiana-produccion.vercel.app" class="cta-button">EXPLORAR COLECCIONES</a>
                        </center>
                    </div>
                ...
                <div class="container">
                    ...
                    <div class="content">
                        ...
                        <div style="text-align: center; margin: 30px 0;">
                            <a href="https://galiana-produccion.vercel.app/seguimiento-paquetes" style="display: inline-block; background: linear-gradient(135deg, #d4af37 0%, #c9a324 100%); color: white; padding: 14px 32px; text-decoration: none; border-radius: 5px; font-weight: 600; font-size: 14px; transition: all 0.3s ease;">
                                Seguir mi Pedido
                            </a>
...
/**
 * Envía alerta de stock a usuarios que tienen el producto en favoritos
 * Tipos: 'stock_bajo' | 'agotado' | 'restock'
 */
export async function sendStockAlertEmail(
    userEmail: string,
    userName: string,
    producto: { nombre: string; imagen?: string; precio: number; id: number },
    tipo: 'stock_bajo' | 'restock',
    stockActual?: number
): Promise<EmailResponse> {

    const siteUrl = 'https://galiana-produccion.vercel.app';
    const productoUrl = `${siteUrl}/productos/${producto.id}`;
    const imagenHtml = producto.imagen
        ? `<img src="${producto.imagen}" alt="${producto.nombre}" style="max-width: 200px; max-height: 200px; object-fit: contain; border-radius: 8px; margin: 0 auto; display: block;" />`
        : '';

    const configs: Record<string, { emoji: string; titulo: string; subtitulo: string; mensaje: string; cta: string; color: string }> = {
        stock_bajo: {
            emoji: '⚡',
            titulo: '¡Últimas unidades!',
            subtitulo: 'Un producto de tus favoritos se está agotando',
            mensaje: `Solo quedan <strong>${stockActual ?? 'pocas'} unidad(es)</strong> disponibles. No dejes escapar esta pieza que tanto te gusta.`,
            cta: 'COMPRAR AHORA',
            color: '#e6a800'
        },
        restock: {
            ...
            subtitulo: 'Un producto de tus favoritos ha vuelto al stock',
            mensaje: '¡Buenas noticias! El producto que te encanta ya está disponible de nuevo. No pierdas la oportunidad, las unidades son limitadas.',
            cta: 'COMPRAR AHORA',
            color: '#28a745'
        }
    };
            ...
            <head>
                ...
                <style>
                    * { margin: 0; padding: 0; box-sizing: border-box; }
                    ...
                    .content { padding: 40px 30px; background: #fafafa; }
                    .greeting { font-size: 18px; color: #d4af37; margin-bottom: 20px; font-weight: 500; }
                    .alert-box { background: white; border-left: 4px solid ${cfg.color}; padding: 20px; margin: 20px 0; border-radius: 3px; text-align: center; }
                    .alert-title { color: ${cfg.color}; font-size: 20px; font-weight: 700; margin-bottom: 8px; }
                    .alert-subtitle { font-size: 14px; color: #666; margin-bottom: 16px; }
                    .product-card { background: white; border: 1px solid #e8dfd4; border-radius: 8px; padding: 24px; margin: 24px 0; text-align: center; }
                    .product-name { font-size: 18px; font-weight: 600; color: #333; margin: 12px 0 8px; }
                    .product-price { font-size: 22px; font-weight: 700; color: #d4af37; margin-bottom: 16px; }
                    .message { font-size: 14px; line-height: 1.8; color: #555; margin: 20px 0; }
                    .cta-button { display: inline-block; background: ${cfg.color}; color: white; padding: 14px 40px; text-decoration: none; border-radius: 3px; font-weight: 600; font-size: 14px; letter-spacing: 1px; }
                    .footer { background: #2c2c2c; color: #d4af37; padding: 30px; text-align: center; font-size: 12px; }
                    .footer p { margin: 8px 0; }
                    .logo-text { font-size: 24px; font-weight: 300; letter-spacing: 1px; }
                </style>
            ...
            <body>
                <div class="container">
                    ...
                    <div class="content">
                        ...
                        <div class="alert-box">
                            <div class="alert-title">${cfg.emoji} ${cfg.titulo}</div>
                            <div class="alert-subtitle">${cfg.subtitulo}</div>
                        </div>

                        <div class="product-card">
                            ${imagenHtml}
                            <div class="product-name">${producto.nombre}</div>
                            ${tipo !== 'stock_bajo' ? `<div class="product-price">€${producto.precio.toFixed(2)}</div>` : ''}
                        </div>
                        ...
                        <center>
                            <a href="${productoUrl}" class="cta-button">${cfg.cta}</a>
                        </center>
                    </div>
                    ...
                    <div class="footer">
                        ...
                        <p>Sanlúcar de Barrameda, España</p>
                        <p style="margin-top: 15px; opacity: 0.7;">© 2026 Joyería Galiana. Todos los derechos reservados.</p>
                        <p style="margin-top: 10px; opacity: 0.5; font-size: 11px;">Recibes este correo porque tienes este producto en tus favoritos.</p>
                    </div>
                </div>
            </body>
        </html>
    ...
    const subjects: Record<string, string> = {
        stock_bajo: `⚡ ¡Últimas unidades de ${producto.nombre}!`,
        agotado: `😔 ${producto.nombre} se ha agotado`,
        restock: `🎉 ¡${producto.nombre} vuelve a estar disponible!`
    };
